...
def load_dataset(csv_file: str):
  ...
  """
  df = pd.read_csv(csv_file)[['business_id', 'name', 'categories', 'latitude', 'longitude', 'stars', 'review_count']]
...
def load_model(model_base: str):
  ...
  """
  nlp = pipeline("feature-extraction", model=model_base, device=0)

  return nlp


def get_embeddings(model, df: pd.DataFrame, column):
  """
...
if __name__ == '__main__':
  ...
  df['exodia_pro'] = df.apply(lambda row: f"{row['name']} {row['latitude']} {row['longitude']} {row['categories']} {row['stars']} {row['review_count']}", axis=1) 
  print(df.head())
  ...
  # Extract Embeddings
  print("\n\nExtract Embeddings...")
  df["embs"] = get_embeddings(model, df, 'exodia_pro')
